...
```
[Global Read] Read RGB pixels from neighborhood into Shared Memory or Registers
      │
      ▼
[Conversion]  Convert neighborhood pixels to YUV format on-the-fly
      │
      ▼
[Filtering]   Run Bilateral YNR and Joint Bilateral CNR using shared coordinates
      │
      ▼
[Conversion]  Convert the filtered YUV value back to RGB
      │
      ▼
[Global Write] Write the final denoised RGB pixel back to Global Memory
```

### Key Performance Benefits
1. **DRAM Bandwidth Savings**: By fusing `RGB->YUV`, `Denoise`, and `YUV->RGB` into a single CUDA kernel, we consume only **1 Read + 1 Write** pass of DRAM bandwidth, compared to 4 passes for separated kernels.
